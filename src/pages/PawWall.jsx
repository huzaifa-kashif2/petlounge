...
export default function PawWall() {
  ...
  const goto = (dir) => {
    ...
    const idxInVisible = visible.findIndex(
      (v) => v.id === items[modalIndex].id
    );
  ...
  const visibleItems = () =>
    filterFav ? items.filter((it) => favorites.has(it.id)) : items;
  ...
  useEffect(() => {
    ...
    const end = (e) => {
      touchRef.current.endX = e.changedTouches
    ...
    <div className={`${styles.outerWrap}`} aria-live="polite">
      ...
      <div className={styles.outerFrame}>
        <div className={styles.innerMat}>
          <div className={styles.pawWallScrollWrapper}>
            <div className={styles.grid}>
              ...
              {tiles.map((it, i) => {
                ...
                return (
                  <article
                    key={it.id}
                    className={`${styles.tile} ${styles[it.size]} ${
                      styles[it.shape]
                    }`}
                  >
                    ...
                    <div className={styles.tileOverlay}>
                      ...
                      <div className={styles.actions}>
                        ...
                        <button
                          className={styles.iconBtn}
                          onClick={() =>
                            downloadImage(it.src, `benny-${it.id}`)
                          }
                        >
                          <FaDownload />
                        </button>
                        <button
                          className={styles.iconBtn}
                          onClick={() => tryShare(it.src, it.title)}
                        >
                          <FaShareAlt />
                        </button>
                      </div>
      ...
      {modalIndex !== null && (
        <div className={styles.modal}>
          ...
          <div className={styles.modalContent}>
            ...
            <button className={styles.modalNavLeft} onClick={() => goto(-1)}>
              <FaChevronLeft />
            </button>
            <button className={styles.modalNavRight} onClick={() => goto(1)}>
              <FaChevronRight />
            </button>
            ...
            <div className={styles.modalInner}>
              ...
              <div className={styles.modalMeta}>
                ...
                <div className={styles.modalActions}>
                  <button
                    onClick={() => toggleFav(items[modalIndex].id)}
                    className={styles.iconBtn}
                  >
                    {favorites.has(items[modalIndex].id) ? (
                      <FaHeart color="#e74c3c" />
                    ) : (
                      <FaRegHeart />
                    )}
                  ...
                  <button
                    onClick={() =>
                      tryShare(items[modalIndex].src, items[modalIndex].title)
                    }
                    className={styles.iconBtn}
                  >
                    <FaShareAlt />
